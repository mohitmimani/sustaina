...
export function TestimonialCard({ testimonial, variants }: TestimonialCardProps) {
  return (
    <motion.div variants={variants} whileHover={{ y: -8, transition: { type: "spring", stiffness: 300 } }}>
      <Card className="h-full rounded-2xl border-none shadow-lg hover:shadow-xl transition-shadow duration-300 backdrop-blur-sm bg-white/80 dark:bg-slate-900/80 border border-white/20 dark:border-slate-700/20">
        <CardContent className="p-6">
          <div className="flex items-center mb-6">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-800 dark:text-white">{testimonial.name}</h3>
