...
export function ProcessStep({ step, isActive, variants }: ProcessStepProps) {
  return (
    <motion.div
      ...
      className={`group ${isActive ? "scale-105" : ""} transition-all duration-300`}
    >
      <Card
        className={`h-full rounded-2xl border-none shadow-lg hover:shadow-xl transition-shadow duration-300 bg-gradient-to-b from-white to-slate-50 dark:from-slate-800 dark:to-slate-900 ${isActive ? "ring-2 ring-emerald-500 dark:ring-emerald-400" : ""}`}
      >
